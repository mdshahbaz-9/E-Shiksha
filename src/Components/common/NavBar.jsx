...
const NavBar = ({ setProgress }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [searchFocused, isMenuOpen]);

  return (
    <>
      <div
        className={`flex fixed top-0 left-0 right-0 bg-slate-900/95 backdrop-blur-md w-full z-50 h-14 sm:h-16 items-center justify-center border-b-[1px] border-b-slate-700/50 transition-all duration-500 shadow-lg ${
          visible ? 'translate-y-0' : '-translate-y-full'
        }`}
      >
        <div className="flex w-[95%] sm:w-11/12 max-w-maxContent items-center justify-between">
          {/* Logo */}
          <Link
            to="/"
            onClick={() => {
          ...
          >
            <img 
              ...
              alt="E-Shiksha" 
              height={36} 
              className="drop-shadow-lg sm:w-[160px] w-[120px]" 
            />
          </Link>

          {/* Desktop Navigation */}
          <nav className="hidden lg:block">
            <ul className="flex flex-row gap-x-6 xl:gap-x-8 text-gray-200 items-center">
              {NavbarLinks?.map((element, index) => (
                <li key={index}>
                  {element.title === "Catalog" ? (
                    <div className="flex items-center group relative cursor-pointer">
                      <p className="font-medium hover:text-cyan-400 transition-colors duration-200 text-sm xl:text-base whitespace-nowrap">
                        {element.title}
                      </p>
                      <svg
                        width="20px"
                      ...
                      <div className="invisible absolute left-[50%] top-[50%] z-[1000] flex w-[220px] translate-x-[-50%] translate-y-[3em] flex-col rounded-xl bg-slate-800 border border-slate-700 p-4 text-gray-200 opacity-0 transition-all duration-200 group-hover:visible group-hover:translate-y-[1.8em] group-hover:opacity-100 lg:w-[280px] xl:w-[300px] shadow-2xl backdrop-blur-md max-h-[400px] overflow-y-auto">
                        <div className="absolute left-[50%] top-0 -z-10 h-6 w-6 translate-x-[80%] translate-y-[-40%] rotate-45 select-none rounded bg-slate-800 border-l border-t border-slate-700"></div>
                        {sublinks?.length > 0 ? (
                          sublinks?.map((element, index) => (
                            <Link
                              to={`/catalog/${element?.name}`}
                              key={index}
                              className="rounded-lg bg-transparent py-3 pl-4 hover:bg-slate-700 hover:text-cyan-400 transition-all duration-200 font-medium text-sm"
                              onClick={() => {
                                dispatch(setProgress(30));
                              }}
                            >
                    ...
                    <Link
                      ...
                      className="font-medium hover:text-cyan-400 transition-colors duration-200 text-sm xl:text-base whitespace-nowrap"
                    >
                      <p
                        className={`${
                          matchRoutes(element?.path)
              ...
              <form onSubmit={handleSearch} className="flex items-center relative">
                <div className="relative">
                  <input
                    ...
                    disabled={isSearching}
                    className={`focus:ring-2 ring-cyan-400 rounded-xl px-3 py-2 pr-16 text-[13px] xl:text-[14px] w-44 xl:w-56 text-gray-200 focus:outline-none focus:border-transparent bg-slate-800 border border-slate-700 placeholder-gray-400 transition-all duration-200 ${
                      isSearching ? 'opacity-70 cursor-not-allowed' : ''
                    } ${searchFocused ? 'ring-2 ring-cyan-400' : ''}`}
                  />
          ...
          {/* Desktop Right Side */}
          <div className="hidden lg:flex flex-row gap-3 xl:gap-4 items-center">
            {user && user?.accountType !== "Instructor" && (
              <Link
                to="/dashboard/cart"
                className="relative px-2 xl:px-3 py-2 rounded-lg hover:bg-slate-800 transition-colors duration-200"
                onClick={() => {
                  dispatch(setProgress(100));
                }}
              >
                <div className="z-50">
                  <TiShoppingCart className="fill-gray-200 hover:fill-cyan-400 w-5 h-5 xl:w-6 xl:h-6 transition-colors duration-200" />
                </div>
                {totalItems > 0 && (
                  <span className="shadow-sm shadow-black text-[10px] font-bold bg-gradient-to-r from-cyan-400 to-teal-500 text-white rounded-full px-1.5 py-0.5 absolute -top-1 -right-1 min-w-[18px] text-center">
                    {totalItems}
                  </span>
                )}
              </Link>
            ...
            {token == null && (
              ...
              >
                <button className="rounded-lg border border-slate-600 bg-slate-800 hover:bg-slate-700 px-3 xl:px-4 py-2 text-gray-200 hover:text-cyan-400 font-medium transition-all duration-200 hover:border-cyan-500 text-sm xl:text-base whitespace-nowrap">
                  Login
                </button>
              </Link>
            )}
            {token == null && (
              ...
              >
                <button className="rounded-lg bg-gradient-to-r from-cyan-500 to-teal-500 hover:from-cyan-600 hover:to-teal-600 px-3 xl:px-4 py-2 text-white font-medium transition-all duration-200 shadow-lg hover:shadow-cyan-500/25 text-sm xl:text-base whitespace-nowrap">
                  Sign Up
                </button>
              </Link>
            )}
          ...
          {/* Mobile/Tablet Right Side - Cart and Menu */}
          <div className="flex lg:hidden items-center gap-2 sm:gap-3">
            ...
            {user && user?.accountType !== "Instructor" && (
              ...
              >
                <TiShoppingCart className="fill-gray-200 w-5 h-5 sm:w-6 sm:h-6" />
                {totalItems > 0 && (
                  <span className="font-medium text-[9px] sm:text-[10px] bg-gradient-to-r from-cyan-400 to-teal-500 text-white rounded-full px-1 sm:px-1.5 py-0.5 absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 min-w-[16px] text-center shadow-lg">
                    {totalItems}
                  </span>
                )}
              </Link>
            ...
            >
              {isMenuOpen ? (
                <IoClose className="w-5 h-5 sm:w-6 sm:h-6 text-gray-200" />
              ) : (
                <GiHamburgerMenu className="w-5 h-5 sm:w-6 sm:h-6 text-gray-200" />
              )}
            </button>
          </div>
        </div>
      ...
      <div
        ref={overlay}
        className="fixed inset-0 z-40 bg-black bg-opacity-60 hidden lg:hidden backdrop-blur-sm"
        onClick={closeMenu}
      ></div>
      ...
      <div
        ref={show}
        className="mobNav fixed top-0 right-0 z-50 h-full w-[85vw] sm:w-80 max-w-[400px] bg-slate-900/95 backdrop-blur-md shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out lg:hidden overflow-y-auto border-l border-slate-700"
      >
        {/* Mobile Menu Header */}
        <div className="flex items-center justify-between p-3 sm:p-4 border-b border-slate-700 bg-slate-800/50 sticky top-0 z-10">
          <img src={logo} width={110} alt="E-Shiksha" className="sm:w-[120px]" />
          <button
            onClick={closeMenu}
            className="p-2 rounded-lg hover:bg-slate-700 transition-colors duration-200"
        ...
        {/* Enhanced Mobile Search */}
        <div className="p-3 sm:p-4 border-b border-slate-700 bg-slate-900/50 sticky top-[57px] sm:top-[65px] z-10">
          <form onSubmit={handleSearch} className="relative">
            <div className="relative">
              <input
                ref={mobileSearchInputRef}
                ...
                placeholder="Search courses..."
                disabled={isSearching}
                className={`w-full px-3 sm:px-4 py-2.5 sm:py-3 pl-9 sm:pl-10 pr-16 sm:pr-20 bg-slate-800 border border-slate-600 rounded-xl text-sm sm:text-base text-gray-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-transparent transition-all duration-200 ${
                  isSearching ? 'opacity-70 cursor-not-allowed' : ''
                }`}
              />
              ...
              <button
                ...
                onClick={handleMobileSearchIconClick}
                disabled={isSearching}
                className={`absolute left-2.5 sm:left-3 top-1/2 transform -translate-y-1/2 transition-colors duration-200 ${
                  isSearching ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer'
                }`}
                title={searchValue.trim().length > 0 ? 'Search' : 'Focus search'}
              >
                {isSearching ? (
                  <div className="animate-spin w-4 h-4 sm:w-5 sm:h-5 border-2 border-cyan-400 border-t-transparent rounded-full"></div>
                ) : (
                  <HiSearch className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400" />
                )}
              </button>
              ...
              <div className="absolute right-1.5 sm:right-2 top-1/2 transform -translate-y-1/2 flex items-center gap-0.5 sm:gap-1">
                ...
                <button
                  type="submit"
                  disabled={isSearching || !searchValue.trim()}
                  className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 shadow-lg ${
                    searchValue.trim() && !isSearching
                      ? 'bg-gradient-to-r from-cyan-500 to-teal-500 text-white hover:from-cyan-600 hover:to-teal-600'
                      : 'bg-slate-700 text-gray-400 cursor-not-allowed'
                  }`}
            ...
            <div key={index}>
              {element.title === "Catalog" ? (
                <div>
                  <button
                    onClick={() => setShowCatalog(!showCatalog)}
                    className="flex items-center justify-between w-full px-3 sm:px-4 py-2.5 sm:py-3 text-left text-gray-200 hover:bg-slate-800 rounded-xl transition-colors duration-200 font-medium text-sm sm:text-base"
                  >
                    <span>{element.title}</span>
                    <svg
                  ...
                  {showCatalog && (
                    <div className="ml-2 sm:ml-4 mt-1.5 sm:mt-2 space-y-1 max-h-52 sm:max-h-60 overflow-y-auto">
                      {sublinks?.length > 0 ? (
                        sublinks.map((subElement, subIndex) => (
                          <Link
                            key={subIndex}
                            ...
                            className="block px-3 sm:px-4 py-2.5 sm:py-3 text-gray-300 hover:text-cyan-400 hover:bg-slate-800 rounded-lg transition-colors duration-200 font-medium text-sm sm:text-base"
                          >
                            {subElement?.name}
                          </Link>
                        ))
                ...
                <Link
                  ...
                  className={`block px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl font-medium transition-colors duration-200 text-sm sm:text-base ${
                    matchRoutes(element?.path)
                      ? "text-cyan-400 bg-slate-800 font-semibold"
                      : "text-gray-200 hover:bg-slate-800 hover:text-cyan-400"
                  }`}
        ...
        {/* Mobile Account Section */}
        <div className="border-t border-slate-700 p-3 sm:p-4 mt-auto sticky bottom-0 bg-slate-900/95 backdrop-blur-md">
          {token !== null ? (
            <div className="space-y-2 sm:space-y-3">
              <div className="text-gray-300 text-xs sm:text-sm font-medium px-3 sm:px-4">
                Account
              </div>
            ...
            <div className="space-y-2 sm:space-y-3">
              <Link
                ...
                className="block"
              >
                <button className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-slate-800 border border-slate-600 text-gray-200 rounded-xl font-medium hover:bg-slate-700 hover:text-cyan-400 hover:border-cyan-500 transition-all duration-200 text-sm sm:text-base">
                  Login
                </button>
              </Link>
              <Link
                ...
                className="block"
              >
                <button className="w-full px-3 sm:px-4 py-2.5 sm:py-3 bg-gradient-to-r from-cyan-500 to-teal-500 text-white rounded-xl font-medium hover:from-cyan-600 hover:to-teal-600 transition-all duration-200 shadow-lg text-sm sm:text-base">
                  Sign Up
                </button>
              </Link>
            </div>
